...
class FilterRules():
    ...
    def init_uniq_key(self,key_obj_list):
        for key_obj in key_obj_list:
            ...
            elif key_obj.mathine_type == "Network device" :
                self.key_dic_map[key_obj.ip] = [key_obj.ip, key_obj.ssh_port, key_obj.ssh_user, key_obj.ssh_passwd,
    ...
    def filter_type(self):
        ...
        for item,item_val in self.key_dic_map.items():
            ...
            if re.search("KVM",item_val[-1]) or re.search("QEMU",item_val[-1]):
                ...
                mac_addr = self.return_key_mac(item_val[-3])
                VirtualServerInfo.objects.create(server_ip=item_val[0],server_type=item_val[-1],system_ver=item_val[6],sys_hostname=item_val[7],mac=mac_addr,sn=item_val[-2],vir_phy_id=1,conn_vir_id=int(self.key_sn_relate[sn_key]))

            elif re.search("Network",item_val[-1]):
                result = item_val[7]
                net_cab = result.split('-')[0]
                net_u = result.split('-')[1]
                net_name = result.split('-')[2]
                e = CabinetInfo.objects.get(cab_name=net_cab)
                cab_id = e.id
                NetWorkInfo.objects.create(host_ip=item_val[0],host_name=net_name,u_lev=net_u,net_cab_id=cab_id)
