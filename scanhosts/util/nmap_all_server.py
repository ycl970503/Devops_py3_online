...
def snmp_begin(nmap_type,ports,password_list,i_key_file,syscmd_list,black_list,s_emails):
    '''
    执行系统主机扫描
    :param nmap_type:
    :param ports:
    ...
    key_login_list,key_not_login_list = nm_item.try_key_login(notlogin_list,i_key_file,syscmd_list)
            ...
            for password in password_list:
                if ip not in self.can_login_lst.keys():
                    ...
                    if res["status"] == "success":
                        ...
                        sys_mac = mac_trans(res["cat /sys/class/net/[^vtlsbd]*/address||esxcfg-vmknic -l|awk '{print $8}'|grep ':'"])
...
#扫描网络设备
class NmapNet:
    def __init__(self,sysname_oid="",community=""):
    ...
    #         #getCmd,setCmd,nextCmd对应snmp里的get-request,set-request,get-next-request报文
    #         #pysnmp做的是snmp的管理进程，所以没有trap，get-response报文的定义。
    #         errorIndication,errorStatus,errorIndex,varBinds = cg.getCmd(
    #             # 社区信息 传入三个数据，安全my-agent、社区名public、snmp协议版本，之间用逗号隔开
    #             cmdgen.CommunityData('server', self.community, 1),
    #             # 通道参数，传入一个元组，包含主机IP/域名和端口
    #             cmdgen.UdpTransportTarget((ip, 161)),
    #             '%s'%sn_oid
    #         )
    #         # varBinds返回是一个stulp，含有MIB值和获得值
    #         print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^", varBinds)
    ...
    def try_log(self,hostname,username,password,port=23):
...
class NmapDocker(NmapDev):
    ...
    def __init__(self,d_cmds,pass_lst,ip_key_dic,ports_lst):
    ...
    def do_nmap(self,host_list):
        ...
        for ip_item in ip_items:
            docker_dct = {}
            tmp_sship_list = []
            doobj = J_ssh_do([ip_item.ip,ip_item.ports_lst,ip_item.ssh_user])
...
def NetDevLogin(dev_ips={},backup_sw="True",back_server=""):
    crpt_do = prpcrypt()
    for ip,login_info in dev_ips.items():
        ...
        if res["status"] == "success" and res["level"] == 2:
            ...
            if dev_obj:
                dev_host_name = dev_obj[0].host_name
                dev_id = dev_obj[0].id

                con_obj = NetConnectionInfo.objects.filter(sn_key=dev_host_name)
                ...
                if not con_obj:
                    print("NetconnTTTDDDDDDDDD",con_user,crpt_pass,en_crpt_pass,ip,dev_host_name,dev_id)
                    NetConnectionInfo.objects.create(tel_username=con_user,tel_userpasswd=crpt_pass,tel_enpasswd=en_crpt_pass,
                    tel_host_port='21',tel_hostip=ip,sn_key=dev_host_name,dev_info_id=dev_id,tel_status=1,tel_type=int(res["level"]))