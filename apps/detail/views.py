...
def machine_list(request):
    ...
    machines = Machines()
    if types == 'cb':
        paginator = Paginator(machines.all_machines(CabinetInfo).order_by("id"), 10)
        template = "detail/cab_list.html"
    if types == 'ad':
        paginator = Paginator(machines.all_machines(CabinetInfo).order_by("id"), 10)
        template = "reports/cab_add.html"
    elif types == 'p':
        paginator = Paginator(machines.all_machines(PhysicalServerInfo).order_by("id"), 10)
        template = "detail/list_phy.html"
    elif types == 'pd':
        paginator = Paginator(machines.all_machines(PhysicalServerInfo).order_by("id"), 10)
        template = "reports/phy_add.html"
    elif types == 'v':
        paginator = Paginator(machines.all_machines(VirtualServerInfo).order_by("id"), 10)
        template = "detail/list_vir.html"
    elif types == 'n':
        paginator = Paginator(machines.all_machines(NetWorkInfo).order_by("id"), 10)
        template = "detail/list_net.html"
    elif types == 'c':
        paginator = Paginator(machines.all_machines(ConnectionInfo).order_by("id"), 10)
        template = "detail/list_conn.html"
    elif types == 'o':
        paginator = Paginator(machines.all_machines(OtherMachineInfo).order_by("id"), 10)
        template = "detail/list_oth.html"
    elif types == 'nc':
        paginator = Paginator(machines.all_machines(NetConnectionInfo).order_by("id"), 10)
        template = "detail/list_netconns.html"
    elif types == 'op':
        paginator = Paginator(machines.all_machines(MachineOperationsInfo).order_by("id"), 10)
...
@login
@level
def list_cab_net(request,param):
    list=[]
    for cab in CabinetInfo.objects.all():
        if cab.cab_name == param:
            print(cab, cab.id,cab.cab_name)
            for net in NetWorkInfo.objects.all():
                if net.net_cab_id == cab.id:
                    list.append([param,net.u_lev,net.host_name,net.host_ip])
                    print(list)
    return render(request, 'detail/list_cab_net.html', {'list': list})