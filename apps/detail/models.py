...
class CabinetInfo(models.Model):
    cab_name = models.CharField(max_length=10, verbose_name=u'机柜编号',unique=True)
    # 1-10分别代表1~10层
    cab_lever = models.IntegerField(verbose_name=u'机器U数,1-10分别代表1~10层')
    cab_width = models.FloatField(max_length=3, verbose_name=u'机柜宽度(米)', default=0.8)
    cab_high = models.FloatField(max_length=3, verbose_name=u'机柜高度(米)', default=2.2)

    cab_deep = models.FloatField(max_length=3, verbose_name=u'机柜深度(米)', default=1)
    cab_area = models.FloatField(max_length=3, verbose_name=u'占地面积(平方米)', default=0.8)
    cab_x = models.FloatField(max_length=5, verbose_name=u'x轴位置')
    cab_y = models.FloatField(max_length=5, verbose_name=u'y轴位置')
...
class PhysicalServerInfo(models.Model):
    ...
    # 物理服务器关联的机柜
    ser_cabin = models.ForeignKey('CabinetInfo',to_field="cab_name")
    # 用户登录系统信息
    # conn_phy = models.ForeignKey('ConnectionInfo')
    phy_u = models.IntegerField(verbose_name=u'安装层数')
...
class VirtualServerInfo(models.Model):
    ...
    system_ver = models.CharField(max_length=30, default='', verbose_name=u'操作系统版本')
    sys_hostname = models.CharField(max_length=80, verbose_name=u'操作系统主机名')
    mac = models.CharField(max_length=512, default='', verbose_name=u'MAC地址')
    sn = models.CharField(max_length=256, verbose_name=u'SN-主机的唯一标识', default='')
...
class NetWorkInfo(models.Model):
    ...
    u_lev = models.IntegerField(verbose_name=u"设备安放U数", default="")

    # 网络设备所在的机柜
    net_cab = models.ForeignKey('CabinetInfo')
    # net_cab = models.CharField(verbose_name=u"机柜编号", default="")
